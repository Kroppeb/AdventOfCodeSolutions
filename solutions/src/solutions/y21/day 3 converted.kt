...
private fun part2() {
	...
	repeat(data[0].size) { i ->
		val oxP = ox.transpose().map { it.countEachI().let { x -> if (x['0'] == x['1']) '1' else x.maxByValue() } }[i]
		val coP = co.transpose().map { it.countEachI().let { x -> if (x['0'] == x['1']) '0' else x.minByValue() } }[i]
