...
@OptIn(kotlin.experimental.ExperimentalTypeInference::class)
@OverloadResolutionByLambdaReturnType
inline fun <T, R> Iterable<T>.rleDecodes(value: (T) -> R, length: (T) -> Sint) = flatMap { listOf(value(it)).repeat(length(it)) }
...
fun <T> Iterable<T>.cartesianPower1(count: Int): List<List<T>> = if (count == 1) this.map { listOf(it) } else cartesianPower1(count - 1).cartesianProduct(this) { a, b -> a + b }
...
inline fun <T1, T2, T3, R> cartesianProductOf(
	v1: Iterable<T1>, v2: Iterable<T2>, v3: Iterable<T3>, transform: (T1, T2, T3) -> R): List<R> =
	v1.flatMap { i1 -> cartesianProductOf(v2, v3) { i2, i3 -> transform(i1, i2, i3) } }

inline fun <T1, T2, T3, T4, R> cartesianProductOf(
	v1: Iterable<T1>, v2: Iterable<T2>, v3: Iterable<T3>, v4: Iterable<T4>, transform: (T1, T2, T3, T4) -> R): List<R> =
	v1.flatMap { i1 -> cartesianProductOf(v2, v3, v4) { i2, i3, i4 -> transform(i1, i2, i3, i4) } }

inline fun <T1, T2, T3, T4, T5, R> cartesianProductOf(
	v1: Iterable<T1>, v2: Iterable<T2>, v3: Iterable<T3>, v4: Iterable<T4>, v5: Iterable<T5>, transform: (T1, T2, T3, T4, T5) -> R): List<R> =
	v1.flatMap { i1 -> cartesianProductOf(v2, v3, v4, v5) { i2, i3, i4, i5 -> transform(i1, i2, i3, i4, i5) } }

inline fun <T1, T2, T3, T4, T5, T6, R> cartesianProductOf(
	v1: Iterable<T1>, v2: Iterable<T2>, v3: Iterable<T3>, v4: Iterable<T4>, v5: Iterable<T5>, v6: Iterable<T6>, transform: (T1, T2, T3, T4, T5, T6) -> R): List<R> =
	v1.flatMap { i1 -> cartesianProductOf(v2, v3, v4, v5, v6) { i2, i3, i4, i5, i6 -> transform(i1, i2, i3, i4, i5, i6) } }

inline fun <T1, T2> cartesianProductOf(
	v1: Iterable<T1>, v2: Iterable<T2>): List<Pair<T1, T2>> =
	cartesianProductOf(v1, v2) { i1, i2 -> i1 to i2 }
...
inline fun <T, R> Iterable<T>.repeatMap(count: Int, mapping: (Int, T) -> R): List<R> = (0 until count).flatMap { i -> map { mapping(i, it) } }

inline fun <T, R> Iterable<T>.repeatMap(count: Sint, mapping: (Sint, T) -> R): List<R> = (0 until count).flatMap { i -> map { mapping(i, it) } }
...
inline fun <T, C : Comparable<C>> Iterable<T>.maxBy(n: Sint, crossinline selector: (T) -> C) = this.sortedByDescending(selector).take(n)

inline fun <T, C : Comparable<C>> Iterable<T>.minBy(n: Sint, crossinline selector: (T) -> C) = this.sortedBy(selector).take(n)
...
inline fun <T, C : Comparable<C>> Iterable<T>.maxOf(n: Sint, selector: (T) -> C) = this.map(selector).sortedDescending().take(n)
...
infix fun <T, R> Collection<T>.splitIn2(transform: (List<T>) -> R): Pair<R, R> = splitIn(2, transform).let { (a, b) -> a to b }
...
// symdiff
infix fun <T> Iterable<T>.symDiff(other: Iterable<T>): Set<T> = LazySet.difference(this, other) or LazySet.difference(other, this)
...
inline fun <T> Iterable<T>.partitionIndexed(predicate: (Int, T) -> Boolean) = withIndex().partition { (i, value) -> predicate(i, value) }.map { l -> l.map { it.value } }


inline fun <T> Iterable<T>.partitionIdx(predicate: (Sint, T) -> Boolean) = withIdx().partition { (i, value) -> predicate(i, value) }.map { l -> l.map { it.value } }
...
inline fun <T, K> Iterable<T>.associateByNotNull(keySelector: (T) -> K?): Map<K, T> = associateBy(keySelector).filterNotNullKeys()

inline fun <T, K, V> Iterable<T>.associateByNotNull(keySelector: (T) -> K?, valueTransform: (T) -> V?): Map<K, V> = associateBy(keySelector, valueTransform).filterNotNullKeys().filterNotNullValues()

inline fun <K, V> Iterable<K>.associateWithNotNull(valueSelector: (K) -> V?): Map<K, V> = associateWith(valueSelector).filterNotNullValues()
